...
- **AND** the textarea uses lined-paper styling
...
The system SHALL allow users to manually add a new expense to a processed receipt. The add expense form SHALL be styled as part of the ticket card.

#### Scenario: Add expense form visible on processed receipts
- **WHEN** a user views a receipt with `status=processed`
- **THEN** an "Ajouter une dépense" form is displayed within the ticket card

#### Scenario: Successful expense creation
- **WHEN** a user submits the add expense form with valid data
- **THEN** a new `Expenses` record is created linked to the receipt

#### Scenario: Validation failure on add
- **WHEN** a user submits the add expense form with invalid data
- **THEN** the form re-displays with error messages

### Requirement: User can edit an existing expense
The system SHALL allow users to edit the details of an existing expense on a processed receipt. Edit controls SHALL be styled consistently with the paper receipt design.

#### Scenario: Edit action visible for each expense
- **WHEN** a user views a receipt with `status=processed`
- **THEN** each expense row has an "Modifier" action styled as a ghost button

#### Scenario: Edit form pre-fills expense data
- **WHEN** a user clicks edit on an expense
- **THEN** the form pre-fills with the current expense values

#### Scenario: Successful expense update
- **WHEN** a user submits the edit expense form with valid data
- **THEN** the expense record is updated

### Requirement: User can delete an expense
The system SHALL allow users to delete an individual expense from a processed receipt. Delete controls SHALL be styled consistently with the paper receipt design.

#### Scenario: Delete action visible for each expense
- **WHEN** a user views a receipt with `status=processed`
- **THEN** each expense row has a "Supprimer" action styled as a ghost button with confirmation

#### Scenario: Successful expense deletion
- **WHEN** a user confirms deletion of an expense
- **THEN** the expense record is removed
