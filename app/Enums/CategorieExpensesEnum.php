<?php

namespace App\Enums;

enum CategorieExpensesEnum: string
{
    case Alimentaire = 'alimentaire';
    case Boissons    = 'boissons';
    case Hygiene     = 'hygiene';
    case Entretien   = 'entretien';
    case Autre       = 'autre';

    public function label(): string
    {
        return match($this) {
            self::Alimentaire => 'Alimentaire',
            self::Boissons    => 'Boissons',
            self::Hygiene     => 'Hygiène',
            self::Entretien   => 'Entretien',
            self::Autre       => 'Autre',
        };
    }

    public function color(): string
    {
        return match($this) {
            self::Alimentaire => 'badge-food',
            self::Boissons    => 'badge-drink',
            self::Hygiene     => 'badge-hygiene',
            self::Entretien   => 'badge-cleaning',
            self::Autre       => 'badge-other',
        };
    }
}